...
public class NSCore4OrdersTabPage extends NSCore4RFWebsiteBasePage{
	...
	private static final By SHIPPING_PROFILE_ADDRESS_LOC=By.xpath("//div[@id='shippingAddressContainer']/div");
	...
	private static String skuBasedProductCVSOOPopupLoc="//table[@id='overrideProducts']//following::td[text()='%s']/following::input[contains(@id,'cvAmount')]";
	...
	public void selectOverrideReasons() {
		...
		dept.selectByIndex(3);
		logger.info("dept select by index 3");
		type.selectByIndex(4);
		driver.pauseExecutionFor(2000);
		logger.info("type select by index 4");
		reason.selectByIndex(5);
		driver.pauseExecutionFor(2000);
		logger.info("type select by index 5");
	}
	...
	public String getShippingTotalFromOrderCreationPage() {
		...
		return driver.getText(SHIPPING_ON_ORDER_CREATION_PAGE_LOC);
	}
	...
	public void enterCVValueInOverrideOrderWindow(String CV,String SKU) {
		driver.pauseExecutionFor(2000);
		driver.findElement(By.xpath(String.format(skuBasedProductQVSOOPopupLoc, SKU))).sendKeys(Keys.CONTROL + "a");
		driver.type(By.xpath(String.format(skuBasedProductQVSOOPopupLoc, SKU)), CV);
