...
public class NSCore4SitesTabPage extends NSCore4RFWebsiteBasePage{
	...
	private static String distributorSubLink = "//h3[contains(text(),'nsDistributor')]/following::li[1]//span[contains(text(),'%s')]";
	...
	private static String nscorporateSubLink = "//a[contains(text(),'%s')]";
	...
	public void selectPreviousMonthFromCurrentDate(){
		...
		prevMonth = driver.getText(By.xpath(String.format(prevMonthLoc,currentMonth)));
		Select select = new Select(driver.findElement(EVENT_MONTH_DD_LOC));
		select.selectByVisibleText(prevMonth);
		logger.info("Selected Month : "+prevMonth);
	}
