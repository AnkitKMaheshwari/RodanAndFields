...
public class StoreFrontAccountInfoPage extends StoreFrontRFWebsiteBasePage{
	...
	public void clickSaveAccountBtn(){
		...
		driver.click(ACCOUNT_SAVE_BUTTON_LOC,"Account Page Save button");
		driver.waitForLoadingImageToDisappear();
		try{
	...
	public StoreFrontOrdersAutoshipStatusPage clickOnAutoShipStatus() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("au")){
	...
	public boolean validateEnterSpouseDetailsAndAccept(){
		...
		driver.clickByJS(RFWebsiteDriver.driver,driver.findElement(By.xpath("//input[@id='acceptSpouse']")),"Accept");
		driver.pauseExecutionFor(2000);
	...
	public void clickOnPcPerksStatus() throws InterruptedException{
		if(driver.getCountry().equalsIgnoreCase("au")){
			this.clickOnWelcomeDropDown();
		}
	...
	public void clickOnGenderRadioButton(String consultantGender) {
		for(int i=0;i<=5;i++){
			...
			driver.clickByJS(RFWebsiteDriver.driver,driver.findElement(By.xpath("//input[@id='"+consultantGender+"']/..")));
			driver.waitForLoadingImageToDisappear();
